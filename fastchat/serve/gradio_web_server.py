...
def load_demo(url_params, request: gr.Request):
    ...
    return (state,
            ...
            gr.Textbox.update(visible=True, value=default_system_text, lines=5),
...
def add_text(state, text, system, request: gr.Request):
    ...
    text = text[:(2048 - len(system))]  # Hard cut-off
    state.append_message(state.roles[0], text)
    state.append_message(state.roles[1], None)
    state.skip_next = False
    return (state, state.to_gradio_chatbot(), "", system) + (disable_btn,) * 5
...
def build_demo():
    with gr.Blocks(title="FastChat", theme=gr.themes.Base(), css=css) as demo:
        ...
        with gr.Column(scale=1):
            systembox = gr.Textbox(show_label=True, label="System",
                visible=False).style(container=False, height=300)

        with gr.Column(scale=1):
            textbox = gr.Textbox(show_label=True, label="User",
                placeholder="Enter text and press ENTER", visible=False).style(container=False, height=200)
        ...
        textbox.submit(add_text, [state, textbox, systembox], [state, chatbot, textbox, systembox] + btn_list
            ).then(http_bot, [state, model_selector, temperature, max_output_tokens],
                   [state, chatbot] + btn_list)
        send_btn.click(add_text, [state, textbox, systembox], [state, chatbot, textbox, systembox] + btn_list
            ).then(http_bot, [state, model_selector, temperature, max_output_tokens],
                   [state, chatbot] + btn_list)

        if args.model_list_mode == "once":
            demo.load(load_demo, [url_params], [state, model_selector,
                chatbot, textbox, systembox, send_btn, button_row, parameter_row],
